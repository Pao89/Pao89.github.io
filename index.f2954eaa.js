...
(function($, window1, document1, undefined) {
    ...
    function Plugin(element, options) {
        ...
        this.elementWidth = window1.getComputedStyle(this.element[0], null).getPropertyValue("width");
        this.elementHeight = window1.getComputedStyle(this.element[0], null).getPropertyValue("height");
        this.elementBackgroundColor = window1.getComputedStyle(this.element[0], null).getPropertyValue("background-color");
    ...
    $.extend(Plugin.prototype, {
        ...
        _flipIt: function(backClass, flipClass) {
            ...
            let $hoverPanel = $("<" + objElement.orginalTagName + ' class="hover _justFlipIt_panel">');
            ...
            let $panelWidth = this.elementWidth, $panelHeight = this.elementHeight, $panelBackgroundColor = this.elementBackgroundColor;
            let $clone = this.element.first().clone().removeClass();
            let $back = $('<div class="' + backClass + " " + originalClassName + '">').append($clone);
            ...
            $.each(this.settings.Style, function(k, v) {
                if (v.el) {
                    if (v.el.toLowerCase() === "self") selfParentHover.css(v.style);
                    else selfParentHover.find(v.el).css(v.style);
                }
            });
            ////REPLACE DEFAULT CONTENT////
            if (this.settings.Template !== "") selfParentHover.find("." + backClass).removeClass(originalClassName).html(this.settings.Template);
            ////ACTION
            if (this.settings.Click === true) selfParentHover.on("click", function(e) {
                e.preventDefault();
                $(this).toggleClass(flipClass);
            });
            else if (typeof this.settings.Click === "string") selfParentHover.find(this.settings.Click).on("click", function() {
                selfParentHover.toggleClass(flipClass);
            });
            else selfParentHover.on("mouseenter", function() {
    ...
    $.fn[_destroy] = function() {
        ...
        this.each(function(k, i) {
            ...
            if ($(this).hasClass("front")) self = $(this);
            else self = $(this).find(".front");
            if (self.siblings().hasClass(objElement.backClass)) {
                self.siblings().remove();
                self.children().attr("class", objElement.oldClasses).unwrap().unwrap();
            } else {
                self.unwrap();
                self.next().remove();
                $(self.firstChild).attr("class", objElement.oldClasses).unwrap();
            }
...
//# sourceMappingURL=index.f2954eaa.js.map
