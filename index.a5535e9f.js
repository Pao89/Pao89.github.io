...
const bouncyCircle = new (0, _coreDefault.default).Shape({
...
function updateBackTopButton() {
    if ($(navbar).isInViewport()) backTopLink.removeClass("shown");
    else backTopLink.addClass("shown");
}
$(document).on("DOMContentLoaded", function() {
    _utils.animateOnWaypoint(".work .card-wrapper", "animate__fadeInUp animate__faster", "100%");
    _utils.animateOnWaypoint(".section__section-heading", "animate__fadeInUp", "100%");
    _utils.animateOnWaypoint(".about-mobile", "animate__fadeInUp", "100%");
    _utils.animateOnWaypoint(".picture-container", "animate__fadeInUp", "100%");
    _utils.animateOnWaypoint(".projects .card", "animate__fadeInUp", "100%");
...
function heroSectionAnimate(event, active = null) {
    ...
    if (isLeftSection) animateLeftSection(isDesktop);
    if (!isLeftSection) animateRightSection(isDesktop);
}
function handleFlipIt() {
    const secondCardTemplate = $("#second-card")[0].outerHTML;
    $("#second-card").detach();
    $("#first-card").justFlipIt({
        Template: secondCardTemplate,
        Click: ".flip-click"
    });
}
...
$(window).on("load", async function() {
    ...
    handleFlipIt();
    checkPosition();
    const repos = await _utils.getGitHubRepos();
    repos.forEach((repo)=>{
...
},{"@mojs/core":"uJH2V","./utils":"02IhW","./spinner":"g8nvH","lodash":"3qBDj","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["ikkB3","1E7ZB"], "1E7ZB", "parcelRequiref510")
